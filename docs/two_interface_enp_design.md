...
This section records how much of the design above is implemented in code. It is intentionally conservative: nothing here claims the two-interface model is validated, and no residual equation below has been solved.

- **Step 1-2 (done, Milestone 1).** `openenpd/two_interface.py` provides the immutable, optimizer-free state assembler (`TwoInterfaceInputs`, `TwoInterfaceState`, `assemble_two_interface_state`, `two_interface_inputs_from_case`). It classifies hard-cutoff species and reports both interface electroneutrality residuals, using the provided Donnan potentials and permeate composition. It does not solve and does not mutate the case.
- **Step 3-4 (done, Milestone 2).** `openenpd/two_interface_residuals.py` builds the residual equations on top of that state: feed-side and permeate-side membrane electroneutrality, permeate electroneutrality, the zero-current condition, and the per-ion ENP flux-closure residuals `J_i^ENP - C_p,i J_v`. The membrane ENP flux reuses `openenpd.enp.enp_flux` with the design's constant-gradient / mean-concentration approximation (§5.4), evaluated with a **provided** membrane potential drop `Δψ_m`. This milestone is residual **construction only**: there is no call to `scipy.optimize.root`/`least_squares`, no parameter fitting, and no change to the baseline solver. Residual magnitudes, an L2 norm, a finiteness flag, the hard-cutoff species, and diagnostic messages are exposed on `TwoInterfaceResiduals`.
- **Two deliberate deviations from §5.8, documented rather than hidden:**
  1. **Reported vs. square vector.** §5.8 specifies a *square* `N+3` system for a `root` solve, which excludes permeate electroneutrality. The Milestone-2 `residual_vector` instead reports the full `4 + N` set (both membrane electroneutralities, permeate electroneutrality, zero current, and `N` flux closures) for transparent auditing. Permeate electroneutrality is flagged in the result messages as a dependent diagnostic (implied by flux closure + zero current for nonzero `J_v`, §5.7); the active-ion set for building the square subset is exposed as `TwoInterfaceResiduals.active_ions`. Forming the square system and dropping the dependent row is left to the Milestone-3 solver.
  2. **Raw, unscaled residuals.** The reported vector mixes units (mol/L for electroneutrality, mol m⁻² s⁻¹ for zero current and flux closure). Dimensionless residual scaling (§6.1) for a well-conditioned solve is deferred to Milestone 3; the L2 norm here is a finiteness/magnitude diagnostic, not a scaled convergence metric.
- **Hard cutoff.** Hard-cutoff species are surfaced (zero membrane concentration, zero ENP flux, flux-closure residual `-C_p,i J_v`) but **not** eliminated from the reported vector; elimination from the active nonlinear vector is a Milestone-3 solver step (§4, step 2/5).
- **Not yet implemented.** Steps 5-10: the `scipy.optimize.root` wrapper, controlled symmetric-electrolyte solve, Foo 2023 smoke solve, four-flux continuation, structural comparison to the baseline, and the spatial BVP. The next milestone is the minimal nonlinear two-interface solver (step 5).
